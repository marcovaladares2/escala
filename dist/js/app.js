...
function loadPersistedState() {
  ...
  state.currentView = ["today", "my-schedule", "general"].includes(preferences.defaultView) ? preferences.defaultView : "today";
  try { if (preferences.lastSelectedDate) parseCivilDate(preferences.lastSelectedDate); } catch { preferences.lastSelectedDate = null; }
  state.selectedDate = state.currentView === "today" ? state.today : (preferences.lastSelectedDate || state.today);
}
...
window.addEventListener("DOMContentLoaded", () => { loadPersistedState(); renderApp(); if (!state.selectedResidentId) openResidentPicker(); if ("serviceWorker" in navigator) navigator.serviceWorker.register("sw.js").catch((error) => console.error("Service worker não registrado:", error)); });
